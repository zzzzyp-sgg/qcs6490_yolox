...
YoloX::YoloX(const std::string &model_path, const int &resolution, 
      const float &score_threshold, const float &nms_threshold, bool quantify)
        : resolution_(resolution), score_threshold_(score_threshold), nms_threshold_(nms_threshold)
{
    ...
    if (config == nullptr)
    {
        LOG(FATAL) << "Create Config failed!\n";
    }
...
bool YoloX::init()
{
    ...
    if (result != 0)
    {
        LOG(FATAL) << "interpreter load model failed!\n";
    }
...
void YoloX::preProcess(const cv::Mat &img, cv::Mat &input_data)
{
    ...
    LOG(INFO) << "scale: " << scale_ << std::endl;
